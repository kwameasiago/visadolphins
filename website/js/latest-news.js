(function () {
    ...
    fetch(APP_CONFIG.API_BASE_URL + '/starred-news.php')
        ...
        .then(function (data) {
            ...
            grid.innerHTML = data.data.map(function (p) {
                ...
                if (p.thumb_type === 'image' && p.thumb_path) {
                    coverHtml = '<div class="news-card__image"><img src="' + resolveUploadUrl(p.thumb_path) + '" alt="" loading="lazy"></div>';
                    ...
                    coverHtml = '<div class="news-card__image"><video src="' + resolveUploadUrl(p.thumb_path) + '" muted></video></div>';
                }
