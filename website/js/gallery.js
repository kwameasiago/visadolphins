(function () {
    ...
    async function loadGallery() {
        ...
        try {
            var res = await fetch(APP_CONFIG.API_BASE_URL + '/gallery.php?' + params.toString());
    ...
    function renderGrid(items) {
        ...
        hexGrid.innerHTML = items.map(function (item) {
            return '<div class="hex-item" data-category="' + escapeAttr(item.tag || '') + '" data-caption="' + escapeAttr(item.caption || '') + '">' +
                '<div class="hex-shape">' +
                    '<img src="' + resolveUploadUrl(item.image_path) + '" alt="' + escapeAttr(item.caption || '') + '" loading="lazy">' +
                '</div>' +
            '</div>';
        }).join('');
