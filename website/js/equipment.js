(function () {
    ...
    async function loadEquipment() {
        ...
        try {
            var res = await fetch(APP_CONFIG.API_BASE_URL + '/equipment.php?' + params.toString());
    ...
    function renderGrid(items) {
        ...
        grid.innerHTML = items.map(function (item) {
            var imgHtml = item.image_path
                ? '<img src="' + resolveUploadUrl(item.image_path) + '" alt="' + escapeHtml(item.name) + '" loading="lazy">'
