(function () {
    ...
    async function loadItem(publicId) {
        try {
            ...
            if (item.image_path) {
                imageDropzone.style.display = 'none';
                imagePreview.innerHTML = '<img src="' + resolveUploadUrl(item.image_path) + '" alt="" style="max-height:180px;border-radius:var(--radius-sm);">' +
    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            ...
            if (!publicId && data.public_id) {
                window.location.href = '/equipment-edit.html?id=' + data.public_id;
            }
        } catch (err) {
            showMessage('Network error.', 'error');
        } finally {
